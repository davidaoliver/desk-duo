...
export default function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-64 bg-dark text-white flex flex-col z-50">
      <div className="p-6 border-b border-dark-light">
        <Link href="/dashboard" className="text-xl font-bold">
          Desk Duo <span className="text-primary-light text-sm font-normal">CRM</span>
        </Link>
      </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-6 py-3 text-sm font-medium transition-colors ${
                  isActive ? "bg-primary text-white" : "text-gray hover:text-white hover:bg-dark-light"
                }`}
              >
                {item.icon}
                {item.label}
      ...
      <div className="p-4 border-t border-dark-light">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray truncate">{user.role.replace("_", " ")}</p>
          </div>
        </div>
        <button
          onClick={() => signOut({ callbackUrl: "/" })}
          className="w-full text-left text-sm text-gray hover:text-white transition-colors"
        >
          Sign out
        </button>
      </div>
